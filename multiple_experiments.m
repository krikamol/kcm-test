...
if length(ns) > 1 && length(delta) == 1 && length(dim) == 1
    ...
    legs = {'KCM','ICM','SMOOTH'};
    generate_power_plot(pwrs,ns,"Sample Size ($n$)","Test Power ($1-\beta$)",[0,1.3],plot_title,legs,strcat(dgp,'_samplesize_test_power.eps'));
...
elseif length(ns) == 1 && length(delta) > 1 && length(dim) == 1
    ...
    legs = {'KCM','ICM','SMOOTH'};
    generate_power_plot(pwrs,delta,"$\delta$","Test Power ($1-\beta$)",[0,1.3],plot_title,legs,strcat(dgp,'_delta_test_power.eps'));
...
elseif length(ns) == 1 && length(delta) == 1 && length(dim) > 1
    ...
    legs = {'KCM','ICM','SMOOTH'};
    generate_power_plot(pwrs,dim,"Dimension of $X$","Test Power ($1-\beta$)",[0,1.3],plot_title,legs,strcat(dgp,'_delta_test_power.eps'));
